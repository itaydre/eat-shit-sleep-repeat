(function (root, factory) {
    ...
    function formatBabyAge(age, gender) {
        ...
        const dayPart = (n) => n + (n === 1 ? ' day' : ' days');
        if (age.unit === 'days') return emoji + dayPart(age.value) + ' old';
        if (age.unit === 'weeks') {
            const w = age.value + (age.value === 1 ? ' week' : ' weeks');
            return emoji + (age.fraction ? w + ' ' + dayPart(age.fraction) : w) + ' old';
        }
        const m = age.value + (age.value === 1 ? ' month' : ' months');
        return emoji + (age.fraction ? m + ' ' + dayPart(age.fraction) : m) + ' old';
    }
